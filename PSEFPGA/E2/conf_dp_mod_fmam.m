...
if ficheros_si == 1
    ...
    fc=sprintf([file_dir 'conf_datain.txt']);
    pack_fc=fopen(fc,'w');
    for i=1:length(s_in)
       fprintf(pack_fc,[num2bin(conf_frec_por,frec_por) ' ' num2bin(conf_im_fm,Kfm) ' ' num2bin(conf_im_am,m_am) ' ' num2str(control_fm_am) '\n']);
    end
    fclose(pack_fc);
